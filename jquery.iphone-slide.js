...
(function() {
  ...
  iphoneslide = (function() {
    ...
    iphoneslide.prototype.fakePage = 1;
    ...
    iphoneslide.prototype._updatepagernav = function() {
      ...
      if (this.isPager === true) {
        return $(opts.pager.selectorName).each(function(i, e) {
          return $("li", $(e)).removeClass(_this.options.pager.childrenOnClass).eq(_this.fakePage - 1).addClass(_this.options.pager.childrenOnClass);
        });
      }
    };
    ...
    iphoneslide.prototype._createpager = function() {
      ...
      $(pagerHtml).delegate("li", "click.iphoneslidepager", function(event) {
        ...
        if (instance.autoPlayerTimer !== null) {
      ...
      if (opts.autoPlay === true) {
        ...
        this.autoPlayerTimer = setInterval(function() {
          if (instance.fakePage !== instance.totalPages) {
    ...
    iphoneslide.prototype._slidetopage = function(easing) {
      ...
      nowPageElem = this.pagesHandler.eq(this.fakePage - 1);
    ...
    iphoneslide.prototype._stopdrag = function(event) {
      ...
      if (m.max(m.abs(this.initDND.origX - stopEvent.pageX), m.abs(this.initDND.origY - stopEvent.pageY)) >= parseInt(opts.sensitivity)) {
        ...
        thisPage = this.pagesHandler.eq(this.fakePage - 1);
        thisPageSize = {
        ...
        switch (opts.direction) {
          case "matrix":
            pageColumn = m.ceil(this.fakePage / this.matrixRow);
            pages.X = pages.X > this.matrixRow ? this.matrixRow : m.abs(this.posDND.X) < thisPageSize.width * opts.draglaunch ? (m.floor(m.abs(easing.Y / easing.X)) > 2 ? 0 : pages.X) : (easing.X > 0 ? m.min(pages.X, this.fakePage - this.matrixRow * (pageColumn - 1)) : m.min(pages.X, this.matrixRow * pageColumn - this.fakePage));
            pages.Y = pages.Y > this.matrixColumn ? this.matrixColumn : m.abs(this.posDND.Y) < thisPageSize.height * opts.draglaunch ? (m.floor(m.abs(easing.X / easing.Y)) > 2 ? 0 : pages.Y) : (easing.Y > 0 ? m.min(pages.Y, pageColumn - 1) : this.matrixRow * pages.Y + this.fakePage > this.totalPages ? this.matrixColumn - pageColumn : pages.Y);
            this.fakePage = easing.X > 0 ? (this.fakePage - pages.X < 1 ? 1 : this.fakePage - pages.X) : (this.fakePage + pages.X > this.totalPages ? this.totalPages : this.fakePage + pages.X);
            this.fakePage = easing.Y > 0 ? (this.fakePage - pages.Y * this.matrixRow < 1 ? 1 : this.fakePage - pages.Y * this.matrixRow) : (pages.Y * this.matrixRow > this.totalPages ? this.totalPages : this.fakePage + pages.Y * this.matrixRow);
            break;
          case "vertical":
            pages.X = 0;
            pages.Y = pages.Y === 0 ? 1 : pages.Y > opts.maxShiftPage ? opts.maxShiftPage : (easing.Y > 0 ? (this.fakePage - pages.Y < 1 ? this.fakePage - 1 : pages.Y) : (this.fakePage + pages.Y > this.totalPages ? this.totalPages - this.fakePage : pages.Y));
            this.fakePage = easing.Y > 0 ? (this.fakePage - pages.Y < 1 ? 1 : this.fakePage - pages.Y) : (this.fakePage + pages.Y > this.totalPages ? this.totalPages : this.fakePage + pages.Y);
            break;
          case "horizontal":
            pages.Y = 0;
            pages.X = pages.X === 0 ? 1 : pages.X > opts.maxShiftPage ? opts.maxShiftPage : (easing.X > 0 ? (this.fakePage - pages.X < 1 ? this.fakePage - 1 : pages.X) : (this.fakePage + pages.X > this.totalPages ? this.totalPages - this.fakePage : pages.X));
            this.fakePage = easing.X > 0 ? (this.fakePage - pages.X < 1 ? 1 : this.fakePage - pages.X) : (this.fakePage + pages.X > this.totalPages ? this.totalPages : this.fakePage + pages.X);
        }
        this.fakePage = this.fakePage >= this.totalPages ? this.totalPages : this.fakePage;
    ...
    iphoneslide.prototype._create = function(options, callback) {
      ...
      $(window).resize(function() {
        if (_this.autoPlayerTimer !== null && opts.cancelAutoPlayOnResize) {
          clearInterval(_this.autoPlayerTimer);
          _this.autoPlayerTimer = null;
        }
        return _this.reset();
      });
    ...
    iphoneslide.prototype._init = function(options) {
      var opts, opts_pager, tmpPage;
      if (typeof options.pager === "object") {
        opts_pager = $.extend({}, defaults.pager, options.pager);
      }
      opts = this.options = $.extend({}, defaults, options);
      opts.pager = this.options.pager = opts_pager;
      tmpPage = this.fakePage || 1;
      ...
      this.pagesHandler.each(function() {
        return $(this).data('realPageNo', $(this).index() + 1);
      });
      this.totalPages = this.pagesHandler.length;
      this._setBoundry();
      this.fakePage = 0;
      this.slide2page(tmpPage, false, false);
    ...
    iphoneslide.prototype.slide2page = function(page, effect) {
      var animate, callCompleted, opts,
        _this = this;
      opts = this.options;
      page || (page = 1);
      effect = typeof effect === "boolean" ? effect : true;
      callCompleted = typeof arguments[2] === "boolean" ? arguments[2] : true;
      if (typeof page === "string") {
        switch (page) {
          case "prev":
            page = this.fakePage - 1;
            break;
          case "next":
            page = this.fakePage + 1;
        }
      }
      if (page <= 0 || page > this.totalPages || page === this.fakePage) {
        return false;
      }
      this.fakePage = page;
    ...
    iphoneslide.prototype.blank2page = function(content, jump2page, callback) {
      ...
      if (content.length > 0) {
        ...
        this.fakePage = jump2page === true ? this.totalPages + 1 : this.fakePage;
        $.each(content, function(index, html) {
          firstElem.clone().removeAttr('style').html(html).appendTo(_this.handler);
          if (index === content.length - 1) {
            return _this._init(opts).slide2page(_this.fakePage);
          }
        });
      }
      return this;
    ...
    iphoneslide.prototype.complete = function() {
      ...
      if (typeof opts.onShiftComplete === "function") {
        ...
        opts.onShiftComplete.apply(this, [pageElem, parseInt(pageElem.data('realPageNo'), 10)]);
      }

...
class $.iphoneslide
  ...
  _updatepagernav: () ->
    ...
    if @isPager is on
      $(opts.pager.selectorName).each(() ->
        $("li", $(@)).removeClass(opts.pager.childrenOnClass).eq(instance.nowPage-1).addClass(opts.pager.childrenOnClass)
      )
  ...
  _createpager: () ->
    ...
    if opts.autoPlay is on
      ...
      @autoPlayerTimer = setInterval(() ->
        if instance.nowPage isnt instance.totalPages then instance.slide2page('next') else instance.slide2page(1)
      , opts.autoPlayTime)
  ...
  _getmovingdata: (pos, init, timestamp) ->
    ...
    ex = if s>w/2 then Math.floor(w/3) else s
  ...
  _stopdrag: (event) ->
    ...
    stopEvent = if touches is undefined then event else (if touches.length > 0 then touches[0] else {pageX: @duringDND.pageX, pageY: @duringDND.pageY, timeStamp: @duringDND.timestamp})
    ...
    if Math.max(Math.abs(@initDND.origX-stopEvent.pageX),Math.abs(@initDND.origY-stopEvent.pageY)) >= parseInt(opts.sensitivity)
      timestamp = Math.abs(stopEvent.timeStamp - @initDND.timestamp)
      ...
      shift = {X:0, Y:0, shift: Math.max(thisMove.X.shift, thisMove.Y.shift), speed: Math.max(thisMove.X.speed, thisMove.Y.speed)}
      easing = {
        X: Math.min(stopEvent.pageX - @initDND.origX, thisPageSize.width),
        Y: Math.min(stopEvent.pageY - @initDND.origY, thisPageSize.height)
      }
      pages = {
        X: if Math.abs(@posDND.X) >= bounce.width*opts.draglaunch or Math.abs(@posDND.Y) >= bounce.height*opts.draglaunch then 0 else if timestamp > opts.touchduring then 1 else Math.ceil(thisMove.X.speed*thisMove.X.shift/thisPageSize.width),
        Y: if Math.abs(@posDND.X) >= bounce.width*opts.draglaunch or Math.abs(@posDND.Y) >= bounce.height*opts.draglaunch then 0 else if timestamp > opts.touchduring then 1 else Math.ceil(thisMove.Y.speed*thisMove.Y.shift/thisPageSize.height)
      }
      during = Math.min(300, opts.touchduring, Math.max(1/shift.speed*Math.abs(opts.extrashift), Math.abs(opts.extrashift)*0.5))

      switch opts.direction
        when "matrix"
          pageColumn = Math.ceil(@nowPage/@matrixRow)
          pages.X = if pages.X>@matrixRow then @matrixRow else if Math.abs(@posDND.X) < thisPageSize.width*opts.draglaunch then (if Math.floor(Math.abs(easing.Y/easing.X))>2 then 0 else pages.X) else 1
          pagex.X = if easing.X>0 then Math.min(pagesX, @nowPage-@matrixRow*(pageColumn-1)) else Math.min(pages.X, @matrixRow*pageColumn-@nowPage)
          pages.Y = if pages.Y>@matrixColumn then @matrixColumn else if Math.abs(@posDND.Y) < thisPageSize.height*opts.draglaunch then (if Math.floor(Math.abs(easing.X/easing.Y))>2 then 0 else pages.Y) else 1
          pages.Y = if easing.Y>0 then Math.min(pages.Y, pageColumn-1) else if @matrixRow*pages.Y+@nowPage>@totalPages then @matrixColumn-pageColumn else pages.Y
        ...
        when "vertical"
          pages.X = 0
          pages.Y = if pages.Y is 0 then 1 else if pages.Y > opts.maxShiftPage then opts.maxShiftPage else pages.Y
          pages.Y = if easing.Y>0 then (if @nowPage-pages.Y<1 then @nowPage-1 else pages.Y) else (if @nowPage+pages.Y>@totalPages then @totalPages - @nowPage else pages.Y)
        ...
        when "horizontal"
          pages.Y = 0
          pages.X = if pages.X is 0 then 1 else if pages.X>opts.maxShiftPage then opts.maxShiftPage else pages.X
          pages.X = if easing.X>0 then (if @nowPage-pages.X<1 then @nowPage-1 else pages.X) else (if @nowPage+pages.X>@totalPages then @totalPages-@nowPage else pages.X)
      ...
      @handler.animate(animate.after, 300, (if $.easing[opts.easing] isnt undefined then opts.easing else "swing"), () ->
        instance.isStartDrag = off
        instance._updatepagernav()
        instance.complete()
      )
    else
  ...
  _startdrag: (event) ->
    ...
    if @isStartDrag is on
      ...
      moveEvent = if touches is undefined then event else touches[0]
  ...
  _initdrag: (event) ->
    ...
    startEvent = if touches is undefined then event else touches[0]
  ...
  _create: (options, callback) ->
    ...
    $(window).resize(() ->
      clearInterval(instance.autoPlayerTimer) if instance.autoPlayerTimer and opts.cancelAutoPlayOnResize
    ...
    switch opts.direction
      when "matrix"
        ...
        for i in [@matrixColumn..1]
          $('<br class="matrix-break-point" style="clear:both;">').insertAfter(
            @pagesHandler.eq((i-1)*@matrixRow-1)
          )
  ...
  slide2page: (page, effect) ->
    ...
    if effect is on
      @handler.animate(animate.after, 300, 
        (if $.easing[opts.easing] isnt undefined then opts.easing else "swing"), 
        () ->
          instance.complete()
      )
    else
  ...
  blank2page: (content, jump2page, callback) ->
    ...
    if content.length > 0
      ...
      $.each(content, (index, html) ->
        firstElem.clone().removeAttr('style').html(html).appendTo(instance.handler)
        if index is content.length-1 then instance._init(opts).slide2page(instance.nowPage)
      )
  ...
  touch: () ->
    ...
    if REGEX.test(userAgent.substr(0,4))
      return yes
    else
      return no
